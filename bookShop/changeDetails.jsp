		...
		<div id="wrap">
			...
			<div id="content">
				<div class="left"> 
					...
					<s:form action="changedministrator" >
						<s:hidden name="id" value="%{administrator.id}"/>
						<s:hidden name="sessionID" value="%{#session.administrator.getId()}"/>
						<s:textfield name="firstName" value="%{administrator.firstName}"  label="Customer First Name" required="true"/>
						<s:textfield name="lastName" value="%{administrator.lastName}"  label="Customer Last Name" required="true"/>
						<s:textfield name="username" value="%{administrator.username}"  label="Customer Username"/>
						<s:password name="password" value="%{administrator.password}" label="Customer Password" required="true"/>
						<s:textarea name="address" value="%{administrator.address}" label="Customer address"  cols="40" rows="10" required="true"/>
						<s:textfield name="age" value="%{administrator.age}" label="Customer Age" required="true"/>
						<s:submit />
					</s:form>
			</div>

			...
			<div id="content">
				<div class="left"> 
					...
					<s:form action="editBook" >
						...
						<s:textfield name="bookName" value="%{book.bookName}"  label="Book Name" required="true"/>
						<s:textfield name="ISBN" value="%{book.ISBN}"  label="Book ISBN" required="true"/>
						<s:textfield name="price" value="%{book.price}"  label="Book Price"/>
						<s:textfield name="stock" value="%{book.stock}" label="Book Stocked Ammount" required="true"/>
						<s:textfield name="manufacture" value="%{book.manufacture}" label="Book Manufacture" required="true"/>
						<s:textfield name="category"  value="%{book.category}" label="Book Category"/>
						<s:datetimepicker label="Publication Date" name="publicationDate" displayFormat="dd/MM/yyyy" required="true"/>
						<s:textfield name="author" value="%{book.author}" label="Book Author" required="true"/>
						<s:submit />
					</s:form>
			</div>
